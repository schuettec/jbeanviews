...
public class BeanViewImpl<S, V> implements BeanView<S, V> {
	...
	public BeanViewImpl(Class<S> sourceType, Class<V> viewType, Set<TypeConversion> typeConversions,
		...
		typeConversions.stream().forEach(
				typeConversion -> this.typeConversions.put(typeConversion.getTypeConversionKey(), typeConversion));
	...
	public V toView(S source) {
		...
		viewBindings.values().stream().forEach(binding -> binding.getSourceValueAsViewValue(view, source));
		return view;
	}
	...
	public Collection<V> toView(Collection<? extends S> source) {
		...
		return (Collection<V>) source.stream().map(s -> toViewOrNull(s)).collect(collector);
	}
	...
	/**
	 * Checks if there is already a view binding for a parent type or property. In
	 * this case, this view binding must not be added.
	 *
	 * @param viewPath The view path.
	 * @return Returns <code>true</code> if there is no parent property binding,
	 *         making this view path obsolete. Returns <code>false</code> otherwise.
	 *
	 */
	private boolean hasParentBinding(String viewPath) {
		return viewBindings.values().stream().map(binding -> binding.getViewPath())
				.filter(anotherViewPath -> Path.startsWith(viewPath, anotherViewPath)).findFirst().isPresent();
	}

	private void removeAllSubPaths(String viewPath) {
		this.viewBindings.entrySet().removeIf(entry -> isViewSubPath(entry.getValue().getViewPath()));
	}
	...
	private void createImplicitViewBindings() {
		...
		viewProperties.keySet().removeIf(path -> {
		...
		Map<String, ViewBinding> implicitViewBindings = viewProperties.entrySet().stream()
				.filter(entry -> !viewBindings.containsKey(entry.getKey())).map(Entry::getValue).map(viewProperty -> {
					Class<?> viewPropertyType = viewProperty.getProperty().getPropertyType();
					String viewPropertyName = viewProperty.getPropertyName();
					// Find matching source property
					Set<TransitiveProperty> candidates = sourceProperties.values().stream().filter(sourceTp -> {
						...
						// or the view type is a collection (in this case the type conversion is
						// validated by the view binding)
					...
					if (candidates.isEmpty()) {
						...
						if (sameLevelCandidates.isEmpty() || sameLevelCandidates.size() > 1) {
							Set<TransitiveProperty> sameNameCandidates = getSameNameCandidates(viewProperty,
									sameLevelCandidates);
							if (sameNameCandidates.isEmpty() || sameNameCandidates.size() > 1) {
								throw ambiguousBindingForProperties(viewProperty,
										getSourcePropertyCandidatesPresentableMessage(viewPropertyName,
												sourceProperties));
							} else {
								TransitiveProperty sourceProperty = sameNameCandidates.iterator().next();
								return createViewBinding(viewProperty, sourceProperty);
							}
						} else {
							TransitiveProperty sourceProperty = sameLevelCandidates.iterator().next();
							return createViewBinding(viewProperty, sourceProperty);
						}
					} else {
						TransitiveProperty sourceProperty = candidates.iterator().next();
						return createViewBinding(viewProperty, sourceProperty);
					}
	...
	private Set<TransitiveProperty> getSameNameCandidates(TransitiveProperty viewProperty,
		...
		Set<TransitiveProperty> sameNameCandidates = sameLevelCandidates.stream()
				.filter(property -> property.getPath().equals(path)).collect(toSet());
	...
	private Set<TransitiveProperty> getSameLevelCandidates(TransitiveProperty viewProperty,
		...
		Set<TransitiveProperty> sameLevelCandidates = candidates.stream()
				.filter(sourceProperty -> viewPropertyLevel == sourceProperty.getLevel()).collect(toSet());
	...
	private ViewBindingImpl createViewBinding(TransitiveProperty viewProperty, TransitiveProperty sourceProperty) {
		...
		// TODO: Verify this!
		boolean isThisBinding = viewProperty.getPropertyType().equals(sourceType);
	...
	private String getSourcePropertyCandidatesPresentableMessage(String viewPropertyName,
		...
		return sourceProperties.values().stream()
				.filter(sourceTp -> viewPropertyName.equals(sourceTp.getPropertyName())).map(tp -> tp.toString(true))
				.collect(joining(",\n"));
	}
	...
	private Map<String, TransitiveProperty> getPropertiesRecursively(Class<?> rootType, Class<?> currentType,
			String path, List<PropertyDescriptor> reflectivePath, Target target) {
		return Properties.getProperties(currentType).stream().map(pd -> {
			...
			if (isCollection(pd.getPropertyType())) {
				...
				/*
				 * If the bean is part of a type conversion (source for source property, dest
				 * for view property), don't add the properties of this bean. Use the bean as
				 * property instead.
				 */
				if (hasTypeConversion(target, propertyType)) {
					// TODO: also add the current reference as property, because otherwise only
					// attribute mapping is possible.
					return asList(new TransitivePropertyImpl(rootType, newPath, newReflectivePath));
				} else {
		...
		}).flatMap(Collection::stream).collect(toMap(TransitiveProperty::getPath, identity()));
	}

	/**
	 * @param target The target type.
	 * @param type   The type to check. For <tt>target=source</tt> it will be
	 *               checked if there is a type conversion having the specified
	 *               source type, for target <tt>target=view</tt> if there is one
	 *               having the specified view type.
	 * @return Returns <code>true</code> if there is a known type conversion having
	 *         the specified type for target.
	 */
	@SuppressWarnings("unchecked")
	private boolean hasTypeConversion(Target target, Class<?> type) {
		...
		if (target == Target.SOURCE) {
			return typeConversions.values().stream().filter(typeConversion -> typeConversion.hasSource(type))
					.findFirst().isPresent();
		} else {
			return typeConversions.values().stream().filter(typeConversion -> typeConversion.hasDestination(type))
					.findFirst().isPresent();
		}
	}
	...
	public String toString() {
		StringBuilder b = new StringBuilder("Bean view ").append(viewType.getName()).append("\n       of ")
				.append(sourceType.getName()).append("\n       with mappings:\n");
		b.append(viewBindings.values().stream().map(ViewBinding::toString).map(s -> "       - " + s)
				.collect(joining(",\n")));
		return b.toString();
	...
	private void validateViewBindings() {
		...
		viewBindings.values().stream().forEach(binding -> {
			try {
				binding.validate();
	...
	boolean isViewSubPath(String partialPath) {
		return viewBindings.values().stream().filter(binding -> Path.startsWith(binding.getViewPath(), partialPath))
				.findFirst().isPresent();
	}

	boolean isViewPropertyPath(String path) {
		return viewBindings.values().stream().filter(binding -> binding.getViewPath().equals(path)).findFirst()
				.isPresent();
	}

	public Set<TypeConversion> getTypeConversions() {
		return this.typeConversions.entrySet().stream().map(Entry::getValue).collect(toSet());
	}
