...
import com.remondis.jbeanviews.test.views.leafTypes.AddressView;

public class ImplicitMappings {

	@Test
	public void shouldMapImplicitlyTransitively() {
		Customer customer = TestData.customer().get();
		BeanView<Customer, CustomerView> beanView = BeanViews.of(Customer.class).toView(CustomerView.class)
				.typeConversion(conversion -> conversion.fromSource(Type.class).toView(String.class)
		...
		assertEquals(customer.getPerson().getForename(), flatCustomer.getForename());
		assertEquals(customer.getPerson().getName(), flatCustomer.getName());
		assertEquals(customer.getType().getKey(), flatCustomer.getType());

		// Restructure values (Push down)
		assertEquals(customer.getNumber(), flatCustomer.getMetaData().getNumber());
		assertEquals(customer.isDeleted(), flatCustomer.getMetaData().isDeleted());

		// Auto creation of views.
		Iterator<AddressView> viewIt = flatCustomer.getAddresses().iterator();
		Iterator<Address> sourceIt = customer.getAddresses().iterator();
